...
class User_model extends CI_Model {
	...
	function getUsersData($user_id){
		if(!empty($user_id)){
		 $sql="SELECT id,first_name,email,user_name,board_id,team_id,last_name  FROM users where is_active=1 and id='".$user_id."'";	
		}else{
		 $sql="SELECT id,first_name,email,user_name,board_id,team_id,last_name  FROM users where is_active=1";
		}
	...
	function loginUser($user_name,$pass_word){
		   ...
		   if($pass_word != '' && $user_name !=''){
			    $query ="SELECT user.id as user_id, user.team_id,user.board_id,team.team_name,user.access_role,CONCAT(user.first_name,' ',user.last_name) as full_name, user.user_name, user.first_name,user.email, team.team_name from users as user INNER JOIN teams as team ON user.team_id = team.id where user_name='".$user_name."' AND pass_word='".$pass_word."'"; 	
			}else {
				$query = "SELECT user.id as user_id, user.team_id,user.board_id,team.team_name,user.access_role,CONCAT(user.first_name,' ',user.last_name) as full_name, user.user_name, user.first_name,user.email, team.team_name from users as user INNER JOIN teams as team ON user.team_id = team.id where user_name='".$user_name."'";
			}
        ...
        foreach($res->result_array() as $loggeddetails){
	...
	function updateChecklistreactupdated($user_id,$task_id,$task_status){
		//print_r($team_id);
		$updateChecklist="update checklist set status=".$task_status." WHERE id=$task_id AND user_id=$user_id";
...
/** fetch specifii team details */
 public function getteamdevsBatch($team_id){
	...
	function addAttendence($user_id,$team_id,$is_absent,$start_date,$end_date,$added_by){
		...
		/** Get Username */
		$firstnameuser ='';
        $passwordChange = "SELECT user_name,first_name from users where id='".$user_id."'";
		$result = $this->db->query($passwordChange);
		 if(!empty($result)){
				foreach($result->result('array') as $userdetails){
					$passdetails = $userdetails["user_name"];
					$firstnameuser = $userdetails["first_name"];
				}
		 }
         /** Add out of office cards */
		 ...
		 if($end_date !=''){
			for($currentDate = strtotime($start_date); $currentDate <= strtotime($end_date); $currentDate += (86400)) {
				...
				$datesArray[] = $start_date;
		 }
        //  echo '<pre>';print_r($datesArray);
         foreach($datesArray as $value){
            if($user_id !=0 && $team_id !=0){
				$clonedetailsquery = "INSERT INTO checklist(user_id,team_id,task_title,ticket_no,task_type,status,planned_date,estimated_time,order_position,is_private,task_category,original_estimate,remaining_estimate,ticket_status,colorcode,is_copied)VALUES('".$user_id."','".$team_id."','OOO','".$firstnameuser."','1','1','".$value."','0.0',1,0,'oof','','','','#eb7bd9',0)";
				$copy_res = $this->db->query($clonedetailsquery);
				if($copy_res == 1){
					 $msg = "Records has been added sucessfully";  
				 } 
			}
		 }
	...
	$availability_in_percentage="SELECT COUNT(ua.id) AS total_absent,(SELECT COUNT(id) from users where is_active=1 AND team_id IN(6,8,9,7,10,11)) total_users,(SELECT COUNT(id) from users where is_active=1 AND team_id IN(6,8,9,7,10,11))- (SELECT COUNT(ua.id)) as present_users,(SELECT ROUND(COUNT(ua.user_id)/COUNT(id)*100,2) from users where is_active=1) absent, (SELECT ROUND(100-COUNT(ua.user_id)/COUNT(id)*100,2) from users where is_active=1) present FROM user_attendence ua WHERE '".$requested_date."' BETWEEN ua.start_date and ua.end_date and ua.is_absent=1 AND ua.team_id IN(6,8,9,7,10,11)";
	...
	function getAttendence($user_id){
		$sql="SELECT * from user_attendence where user_id = ? AND end_date>=CURRENT_DATE()";
  ...
  public function postNotificationdetails($added_id,$board,$teamid,$message,$startDate,$endDate){
	...
	for($i=0;$i<count($teamid);$i++){
     ...
     $res = $this->db->insert_batch('notify', $insertarr);
  ...
  public function getDatesList($user_id,$currentuser){
	...
	$db_query = "SELECT user_id,planned_date,task_title,ticket_no,status,task_type,CAST(estimated_time AS FLOAT) AS estimated_time,id,order_position,is_private,task_category,original_estimate,remaining_estimate,ticket_status,colorcode FROM checklist WHERE user_id='".$user_id."' ORDER BY order_position"; 
	...
	$res = $this->db->query($db_query);
	 ...
	 $colorcode_array = array(); 
foreach($res->result('array') as $getspecificdetails){
//   $colorcode_array[$getspecificdetails['planned_date']][$getspecificdetails['id']] = $getspecificdetails['colorcode'];
...
public function getCloneDetails($ticketClone){
	...
	/** get the task details to be copied */
	$user_id   = $ticketClone['user_id'];
	$event_type = $ticketClone['task_type'];
	$team_id   = $ticketClone['team_id'];
	$summary   = addslashes($ticketClone['task_title']);
...
public function getSortupdateDetails($details,$user_id){
	...
	for($i=0;$i<count($value);$i++){
		 $update = "UPDATE checklist set order_position ='".($i+1)."',planned_date='".$value[$i]['planned_date']."' WHERE id='".$value[$i]['id']."' and user_id='".$user_id."'";
...
/** End Query Updated **/
	...
	for($i=0;$i<count($bargraphdates);$i++){
		$absentbargraph ="SELECT COUNT(*) As no_of_users, '".$bargraphdates[$i]."' as selecteddate FROM `user_attendence` WHERE '".$bargraphdates[$i]."' BETWEEN start_date AND end_date AND team_id in (6,8,9,7,10,11)";
 ...
 /* public function getAddeddetailsSpecific($insertdetails){
    ...
    from checklist WHERE team_id in (6,7,8,9,10,11) AND planned_date='".$requested_date."'";
	/** end modified */
    ...
    INNER JOIN users u
    ON u.id=ua.user_id
    WHERE '".$requested_date."' BETWEEN ua.start_date AND ua.end_date AND ua.team_id IN(6,7,8,9,10,11)";
    
	$availability_in_percentage="SELECT COUNT(ua.id) AS total_absent,
    (SELECT COUNT(id)   from users where is_active=1 AND team_id IN(6,8,7,9,10,11)) total_users,
    (SELECT ROUND(COUNT(ua.id)/COUNT(id)*100,2) from users where is_active=1 AND team_id IN(6,8,9,7,10,11)) absent_percentage,
    (SELECT ROUND(100-COUNT(ua.id)/COUNT(id)*100,2) from users where is_active=1 AND team_id IN(6,8,9,7,10,11)) present_percentage
	...
	// 	IF(DATEDIFF(ua.end_date, ua.start_date) = 0, 1, DATEDIFF(ua.end_date, ua.start_date)) + 1 as no_of_days
	// 	from user_attendence ua
	// 	INNER JOIN users u
	// 	ON u.id=ua.user_id
	// 	WHERE ua.user_id='".$user_id."'";
	$absent_list="select CONCAT(u.first_name,' ',u.last_name) as user_name, ua.start_date, ua.end_date,
		DATEDIFF(ua.end_date, ua.start_date) + 1 as no_of_days
		from user_attendence ua
		INNER JOIN users u
		ON u.id=ua.user_id
		WHERE ua.user_id='".$user_id."'";
...
function absentCalendarSummary($details){
	
$teamid ="'".implode("','", explode(",", $details[0]["teamid"]))."'";

$dbquery_res = "select CONCAT(u.first_name,' ',u.last_name) as user_name,ua.start_date,ua.end_date,IF(DATEDIFF(ua.end_date, ua.start_date) = 0, 1, DATEDIFF(ua.end_date, ua.start_date)) as no_of_days from user_attendence ua INNER JOIN users u  ON u.id=ua.user_id WHERE ( (ua.start_date BETWEEN '".$details[0]["startdate"]." 00:00:00' and '".$details[0]["endate"]." 00:00:00') OR (ua.end_date  BETWEEN '".$details[0]["startdate"]." 00:00:00' and '".$details[0]["endate"]."')) and ua.team_id IN(".$teamid.")";
$res = $this->db->query($dbquery_res)->result_array();
if(!empty($res)){
	$msg = ['status'=>true,"absentcalendarlist"=>$res];
}else{
	$msg = ['status'=>false,"msg"=>"No Data Found"];
}
return $msg;
...
function getBargraphDetails($username,$teamid,$startdate,$enddate){
    ...
    $getbargraphdetails = array();
    $datateam = implode("','",$teamid);
    $countteamid = "SELECT COUNT(*) as totalusers FROM users where team_id IN ('".$datateam."') and is_active=1";
	
    $countresults = $this->db->query($countteamid)->result_array();
    // echo ;exit;
	
    $countdetails ='';
    $countdetails = $countresults[0]['totalusers'];
    $totalhours='';
    ...
    for($i=0;$i<count($attendance);$i++){
       $teamidetails = "SELECT count(ua.is_absent) as totalusers,date('".$attendance[$i]."') as planned_date from user_attendence ua where ua.team_id IN ('".$datateam."') and date('".$attendance[$i]."') BETWEEN start_date and end_date";
    ...
    if(!empty($teamid)){
       // $conditon1 ="or c.team_id IN ('".$datateam."')";
       $conditon1 =" and c.team_id IN ('".$datateam."')";
        $teamcond1 = ",round('".$countdetails."'*8.5,2) as hoursperday";
        $selectcond1 = "c.team_id";
    }else{
        $conditon1 = '';
	...
	$dbfilter ="SELECT  '".$selectcond1."' '".$selectcond2."',sum(case when c.status = 1 then 1 else 0 end) as completedtasks, sum(case when c.status <> 1 then 1 else 0 end) as pendingtasks,sum(case when c.status = 1 then 1 when c.status = 0 then 1 else 0 end) as totaltasks,round(sum(case when c.status = 1 then estimated_time else 0 end),2) as completedhours,round(sum(case when c.status = 0 then estimated_time else 0 end),2) as pendinghours,c.planned_date as date,round(SUM(estimated_time),2) as hours_planned $teamcond1 $teamcond2 from checklist c inner join users u on u.id = c.user_id $condition2 $conditon1  AND c.planned_date BETWEEN '".$startdate."' AND '".$enddate."' GROUP by c.planned_date,'".$selectcond1."' '".$selectcond2."'";
     ...
     $countresults = $this->db->query($dbfilter);
     $datefetchedarray = array();
     $datefetchedarray = $countresults->result("array");
     ...
     foreach($datefetchedarray as $records){
        //  echo $records["date"];
     ...
     if(!empty($bargraphdetails_arr)){
      ...
      return $bargraphdetails_arr;
		...
		public function addUserJiraAcessKeys($user_id,$jira_access_key){
			$sql = "INSERT INTO user_jira_access_keys(user_id,jira_access_key)VALUES(?,?)";
			$res = $this->db->query($sql,[$user_id,$jira_access_key]);
			$msg='';
				if($res == 1){
...
public function getJiraTicket($ticketNo){
	...
	if(curl_errno($ch)) {
        ...
        // $colorcodejira = array('blue-gray'=>'#6787F1','yellow'=>'#e98224','green'=>'green');
...
public function cql_ticket($user_id,$label,$team_member_id,$start_date,$end_date){
	$sql="SELECT CONCAT(u.first_name, ' ', u.last_name) AS user_name,
	c.ticket_no as label,
	c.task_title,
	c.estimated_time as logged_hours,
	c.planned_date,
	IF(c.task_type=1, 'Task', 'Meetings') as task_type,
	IF(c.status=0, 'In Progress', 'Completed') as task_status,
	IF(c.is_private=0, 'No', 'Yes') as task_privacy,
	c.date_added,
	c.date_updated
	FROM checklist c
	INNER JOIN users u
	...
	if (!empty($label)) {
	$conditions[] = "c.ticket_no = '$label'";
	}
	if (!empty($user_id) && $user_id !="-1") {
	$conditions[] = "c.user_id = '$user_id'";
	}
	if (!empty($start_date) && !empty($end_date)) {
	$conditions[] = "c.planned_date BETWEEN '$start_date' AND '$end_date'";
	}
	if (!empty($conditions)) {
...
//    echo $sql;  
   ...
   if(!empty($result)):
	$msg = ['status'=>true,"user_data"=>$result];
	else:
	$msg = ['status'=>false,'msg'=>'Something went wrong'];
	endif;
...
public function cql_label_ticket($ticket_no,$start_date,$end_date){
    $sql = "SELECT CONCAT(u.first_name, ' ', u.last_name) AS user_name, c.ticket_no as label, c.task_title, c.estimated_time as logged_hours, c.planned_date, IF(c.task_type=1, 'Task', 'Meetings') as task_type, IF(c.status=0, 'In Progress', 'Completed') as task_status, IF(c.is_private=0, 'No', 'Yes') as task_privacy, c.date_added, c.date_updated from checklist c INNER JOIN users u ON u.id = c.user_id where ticket_no='".$ticket_no."' AND planned_date BETWEEN '".$start_date."' AND '".$end_date."'";
...
//pending Tasks
// public function showpendingchecklistItems($previous_date,$user_id){
// 	$sql = "SELECT count(status) as pendingcount from checklist where user_id='".$user_id."' and planned_date='".$previous_date."' and status=0";
	
// 	$res = $this->db->query($sql);
// 	$count = '';
// 	foreach($res->result('array') as $records){
...
public function showpendingchecklistItems($previous_date,$user_id){
	$sql = "SELECT count(status) as pendingcount,planned_date as pendingitemsdate from checklist where user_id='".$user_id."' and planned_date='".$previous_date."' and status=0";
	
	$res = $this->db->query($sql);
	$pendingtasks = array();
	foreach($res->result('array') as $records){
		  $pendingtasks = array('pendingcount'=>$records["pendingcount"],'pendingitemsdate'=>$records["pendingitemsdate"]);
	}
	
	return $pendingtasks;
}

function userReports($user_id,$label,$team_member_id,$start_date,$end_date){
...
if (!empty($team_member_id)) {
 $conditions[] = "c.user_id = '$team_member_id'";
}
if (!empty($start_date) && !empty($end_date)) {
 $conditions[] = "c.planned_date BETWEEN '$start_date' AND '$end_date'";
}
...
#Forget Reset Mail Functionality
public function findemail($email,$random){

   $sql = "SELECT count(*) as userpresent from users where email='".$email."' and is_active=1";
   $res = $this->db->query($sql)->result('array');
   
    $msg= [];   
    if($res[0]['userpresent'] == 0){
         $msg  = ['status'=>0,"msg"=>"Email doesnot Exists"];
     	}else{
		  $db_query = "INSERT INTO password_verification(email_id,verification_code)VALUES('".$email."','".$random."')";
		  $res = $this->db->query($db_query);	
          $msg = ['status'=>1,"msg"=>"Mail Sent Successfully"]; 
  	}   
...
//resetPassword
public function updateUserPassword($verifcation_token,$email){
    $verified=[];
	$query_verify = "SELECT count(*) as verification_status from password_verification where verification_code='".$verifcation_token."' and email_id='".$email."' and is_active=1";
  
   $res_1 = $this->db->query($query_verify)->result('array');
  
   if($res_1[0]['verification_status'] == 1){
	   $verified = ['status'=>1,'email'=>$email];
   }else {
	   $verified = ['status'=>0,'email'=>$email];
   }
   return $verified;
}


public function newPasswordgenerator($reset,$email){
    $query = "UPDATE users set pass_word ='".md5($reset)."' where email='".$email."' and is_active=1";
	$res_1 = $this->db->query($query);
	if($res_1 === true){
		return 1; 
	}
	// if($res_1) 
...
//reset password Mail
function passwordreseter($status,$email,$token){
  if($status == 1){
    $resetpassword = substr(str_shuffle(str_repeat("0123456789abcdefghijklmnopqrstuvwxyz", 5)), 0, 5);
    
    $passowrdresettest =$this->newPasswordgenerator($resetpassword,$email); 
	if($passowrdresettest === 1){
	$db_query = "UPDATE password_verification set is_active=0 where email_id='".$email."' and verification_code='".$token."'";
	$res = $this->db->query($db_query);
	if($res === true){
	...
	// Email subject
	$mail->Subject = 'Your new password [checklistsupport@lenovo.com]';
	...
	// Email body content
	$mailContent = "<h4>Your Password has been reset successfully</h4>
		<p>Please don,t Share Your Password with Anyone and please change your account password once logged in</p>
		 <p>Your Password to Login : <b>$resetpassword</b></p> 
		";
...
/** Developer Status */
public function developerStatus($currentDate){
   $db_query = "SELECT user_id, ROUND(SUM(estimated_time), 2) AS hours_planned, 8.5 AS available_hours,ROUND(SUM(CASE WHEN status IN (0, 1) THEN estimated_time ELSE 0 END), 2) AS added_hours, ROUND(8.5 - SUM(CASE WHEN status IN (0, 1) THEN estimated_time ELSE 0 END), 2) AS remaining_hours FROM checklist WHERE team_id IN (6, 7, 8, 9, 10, 11) AND planned_date ='".$currentDate."' GROUP BY user_id";	
   $res = $this->db->query($db_query)->result_array();
   $getUseravailablestatus = array(); 
    
   $getuserdetails = "SELECT id as user_id,concat(first_name,' ',last_name) as username from users where is_active = 1";
   $userdetails = $this->db->query($getuserdetails)->result_array();
 
   $usernames = array();
   foreach($userdetails as $details){
	  $usernames[$details['user_id']] = $details['username'];
   }
   
  // $status = array();
   /** get Avialable Status */

 
   if(!empty($res) && isset($res)){
	foreach($res as $userstatus) {
		$getUseravailablestatus[] = array(
			'username'=> $usernames[$userstatus['user_id']],
			'totalhours'=> $userstatus['available_hours'],
			'addedhours'=> $userstatus['added_hours'],
			'remaininghours'=> $userstatus['remaining_hours']
		);
	}
    
   }
   if(!empty($getUseravailablestatus)){
	  return json_encode($getUseravailablestatus);
   }  
