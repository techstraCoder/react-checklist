...
abstract class REST_Controller extends CI_Controller {
    ...
    protected function _detect_api_key()
    {
        ...
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name)))
        {
            if(count(explode('-', $key))==3){
                list($apikey,$userid,$randomstry) = explode('-', $key);
                $userid = ($userid-1000);
                $this->user_id = $userid;
                $_POST['userid'] = $this->user_id;
                if ( ! ($row = $this->rest->db->where(
                                                      [
                                                        $this->config->item('rest_key_column')=>$apikey,
                                                        $this->config->item('rest_user_colomn')=>$userid,
                                                        'is_login'=>1
                                                      ]
                                                      )->get($this->config->item('rest_keys_table'))->row()))
                {
                    if($apikey=='123456' && $userid='7'):
                        return true;
                    else:
                        return FALSE;
                    endif;
    ...
    protected function genratekey($userid=3){
       $alph = ['A','B','C','D','E','F','G','H','I','J'];
       $alph_str = array_rand($alph,6);
       $userid_str = $userid+1000;
       $randnum = rand(000000,999999);
       $key = implode("", $alph_str);
       $key_str = implode("", $alph_str)."-".$userid_str."-".$randnum;
       $serverIP = $this->input->server('REMOTE_ADDR');
       $insert['user_id'] = $userid;
       $insert['level'] = 0;
       $insert['ignore_limits'] = 0;
       $insert['ip_addresses'] = $serverIP;
       $insert['key'] = $key;
       $insert['date_created'] = strtotime(date('Y-m-d H:i:s'));
       $row = $this->rest->db->where([
                                   'ip_addresses'=>$serverIP,
                                   'user_id'=>$userid,
                                   'is_login'=>0
                                 ])->get($this->config->item('rest_keys_table'))->row_array();
       if(!$row){
            $this->rest->db->insert($this->config->item('rest_keys_table'),$insert);
       }else{
            $insert['is_login']=1;
            $this->rest->db->where([
                                   'id'=>$row['id']
                                  ]);
            $this->rest->db->update($this->config->item('rest_keys_table'),$insert);
       }
    ...
    function userlogout(){
        ...
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name)))
        {
            if(count(explode('-', $key))==3){
                ...
                $logout = $this->rest->db->delete($this->config->item('rest_keys_table'));
                if($logout){
                      return true;
                }else{
                    return false;
